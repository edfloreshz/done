...
use relm4::{
    ...
    gtk::{self, traits::WidgetExt, prelude::ListBoxRowExt},
...
impl AsyncFactoryComponent for ServiceFactoryModel {
    ...
    async fn update(
        ...
        match message {
            ServiceFactoryInput::Selected => {
                sender.output(ServiceFactoryOutput::ServiceSelected(self.service.clone()));
    ...
    fn forward_to_parent(output:Self::Output) -> Option<Self::ParentInput> {
        let output = match output {
            ServiceFactoryOutput::ServiceSelected(service) => ServicesSidebarInput::ServiceSelected(service),
        };
        Some(output)
    }
}